...
if __name__ == '__main__':
    ...
    wandb_run_id = 'wrc2h54x' #first fema very good
    ...
    stages = ['1','2','3']
    #get wandb run id
    res_name="avg_FemaFinal_checpoit"+wandb_run_id



    resultsdir = os.path.join('./ressss', res_name)
    ...
    for stage in stages:
        ...
        ds2 = FaceFramesSeqPredictionDataset(os.path.join(test_labels_dir, name),dataset_root,transform=transform_test,seq_len=seq_len)
        ds3 = FaceFramesSeqPredictionDataset(os.path.join(test_labels_dir, name),dataset_root,transform=transform_test,seq_len=seq_len)
        ...
        with torch.no_grad():
            for x,nameee in ds:
                ...
                test_labels1.append(y)
                test_names1.append(nameee)

        print(f"predicted {len(test_labels1)} labels for {name}")
        print(f'len test_names {len(test_names1)}')
        print(f'len test_labels {len(test_labels1)}')
        ...
        with torch.no_grad():
            for x,nameee in ds:
                ...
                test_labels2.append(y)
                test_names2.append(nameee)

        print(f"predicted {len(test_labels2)} labels for {name}")
        print(f'len test_names {len(test_labels2)}')
        print(f'len test_labels {len(test_labels2)}')
        ...
        with torch.no_grad():
            for x,nameee in ds:
                ...
                test_labels3.append(y)
                test_names3.append(nameee)

        print(f"predicted {len(test_labels3)} labels for {name}")

        #calculate std between 3 predictions
        test_labels = []
        ...
        #calculate std
        std = np.std([test_labels1,test_labels2,test_labels3],axis=0)

        print(f'std {std}')

        #calculate mean
        mean = np.mean([test_labels1,test_labels2,test_labels3],axis=0)
        #calculate avg
        test_labels=mean

        #calculate rmse bbetwen predictions
        rmse1 = np.sqrt(np.mean((test_labels1 - test_labels2)**2))
        rmse2 = np.sqrt(np.mean((test_labels1 - test_labels3)**2))
        rmse3 = np.sqrt(np.mean((test_labels2 - test_labels3)**2))
